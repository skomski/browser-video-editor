/*
 ...
 License GNU:
 https://github.com/danielcebrian/rangeslider-videojs/blob/master/License-GNU.rst
 License Apache:
 https://github.com/danielcebrian/rangeslider-videojs/blob/master/License-Apache.rst
 */
...
(function() {
  ...
  function RangeSlider_(options) {
    ...
    function initialVideoFinished(event) {
      ...
      // All components will be initialize after they have been loaded by
      // videojs
      ...
      plugin._reset();
      player.trigger(
          'loadedRangeSlider'); // Let know if the Range Slider DOM is ready
    }
  ...
  function RangeSlider(player, options) {
    ...
    if (!options.hasOwnProperty('controlTime'))
      options.controlTime =
          true; // Show Control Time to set the arrows in the edition
  ...
  RangeSlider.prototype = {
    ...
    init : function() {
      ...
      this.right = this.components.SelectionBarRight =
          this.box.SelectionBarRight;
      ...
      this.ctpl = this.components.ControlTimePanelLeft =
          this.ctp.ControlTimePanelLeft;
      this.ctpr = this.components.ControlTimePanelRight =
          this.ctp.ControlTimePanelRight;
    ...
    lock : function() {
      ...
      if (typeof this.box != 'undefined')
        addElClass(this.box.el_, 'locked');
    },
    unlock : function() {
      this.options.locked = false;
      this.ctp.enable();
      if (typeof this.box != 'undefined')
        removeElClass(this.box.el_, 'locked');
    },
    show : function() {
    ...
    showPanel : function() {
      ...
      if (typeof this.tp != 'undefined')
        removeElClass(this.tp.el_, 'disable');
    },
    hidePanel : function() {
      this.options.panel = false;
      if (typeof this.tp != 'undefined')
        addElClass(this.tp.el_, 'disable');
    },
    ...
    setValue : function(index, seconds, writeControlTime) {
      // index = 0 for the left Arrow and 1 for the right Arrow. Value in
      // seconds
    ...
    loop : function(start, end, show) {
      ...
      if (player) {
        player.on("pause",
                  videojs.bind(this, function() { this.looping = false; }));
    ...
    _getArrowValue : function(index) {
      ...
      var percentage =
          this[index === 0 ? "left" : "right"].el_.style.left.replace("%", "");
      if (percentage == "")
        percentage = index === 0 ? 0 : 100;

      return (this._seconds(percentage / 100))
          .toFixed(this.updatePrecision - 1);
    },
    ...
    _setValuesLocked : function(start, end, writeControlTime) {
      ...
      if (this.options.locked) {
        this.unlock(); // It is unlocked to change the bar position. In the end
    ...
    _checkControlTime : function(index, TextInput, timeOld) {
      var h = TextInput[0], m = TextInput[1], s = TextInput[2],
          newHour = h.value, newMin = m.value, newSec = s.value, obj, objNew,
          objOld;
      index = index || 0;
      ...
      var duration = this.player.duration() || 0, durationSel;
      ...
      if (index === 1) {
        var oldTimeLeft = this.ctpl.el_.children,
            durationSelLeft = videojs.TextTrack.prototype.parseCueTime(
                oldTimeLeft[0].value + ":" + oldTimeLeft[1].value + ":" +
                oldTimeLeft[2].value);
        ...
        var oldTimeRight = this.ctpr.el_.children,
            durationSelRight = videojs.TextTrack.prototype.parseCueTime(
                oldTimeRight[0].value + ":" + oldTimeRight[1].value + ":" +
    ...
    _triggerSliderChange : function() { this.player.trigger("sliderchange"); }
  };
  ...
  // Lock the Slider bar and it will not be possible to change the arrow
  // positions
  ...
  videojs.getComponent('SeekBar')
      .prototype.options_.children.push('RSTimeBar'); // Range Slider Time Bar
  videojs.getComponent('ControlBar')
      .prototype.options_.children.push(
          'ControlTimePanel'); // Panel with the time of the range slider
   ...
   * @constructor
   */
  videojs.RSTimeBar = videojs.extend(Component, {
    /** @constructor */
  ...
  videojs.RSTimeBar.prototype.options_ = {children : {'SeekRSBar' : {}}};

  videojs.RSTimeBar.prototype.createEl = function() {
    return Component.prototype.createEl.call(
        this, 'div', {className : 'vjs-timebar-RS', innerHTML : ''});
  };
   ...
   * @constructor
   */
  videojs.SeekRSBar = videojs.extend(videojs.getComponent('Component'), {
    /** @constructor */
    constructor : function(player, options) {
      Component.call(this, player, options);
  ...
  videojs.SeekRSBar.prototype.createEl = function() {
    return Component.prototype.createEl.call(
        this, 'div', {className : 'vjs-rangeslider-holder'});
  };
  ...
  videojs.SeekRSBar.prototype.setPosition = function(index, left,
    ...
    // Alias
    var ObjLeft = this.rs.left.el_, ObjRight = this.rs.right.el_,
        Obj = this.rs[index === 0 ? 'left' : 'right'].el_,
        tpr = this.rs.tpr.el_, tpl = this.rs.tpl.el_, bar = this.rs.bar,
    ...
    if ((index === 0 ? bar.updateLeft(left) : bar.updateRight(left))) {
      ...
      // Fix the problem  when you press the button and the two arrow are
      // underhand
      ...
      if (index === 0) {
        ...
        if ((tpr.style.left.replace("%", "") -
             tpl.style.left.replace("%", "")) <= MaxDisP)
          tpl.style.left =
              Math.max(MinP, Math.min(MaxP, tpr.style.left.replace("%", "") -
                                                MaxDisP)) +
              '%';
        ...
        if (((tpr.style.left.replace("%", "") || 100) -
             tpl.style.left.replace("%", "")) <= MaxDisP)
          tpr.style.left =
              Math.max(MinP, Math.min(MaxP, tpl.style.left.replace("%", "") -
                                                0 + MaxDisP)) +
              '%';
  ...
  videojs.SeekRSBar.prototype.getRSTBX = function() {
    return findElPosition(this.el_).left;
  };
   ...
   * @constructor
   */
  videojs.SelectionBar = videojs.extend(videojs.getComponent('Component'), {
    /** @constructor */
    constructor : function(player, options) {
      Component.call(this, player, options);
  ...
  videojs.SelectionBar.prototype.createEl = function() {
    return Component.prototype.createEl.call(
        this, 'div', {className : 'vjs-selectionbar-RS'});
  };
  ...
  videojs.SelectionBar.prototype.onMouseUp = function() {
    var start = this.rs.left.el_.style.left.replace("%", ""),
        ...
        duration = this.player_.duration(), precision = this.rs.updatePrecision,
        segStart = (start * duration / 100).toFixed(precision),
        segEnd = (end * duration / 100).toFixed(precision);
  ...
  videojs.SelectionBar.prototype.updateLeft = function(left) {
    ...
    var width =
        (right - left)
            .toFixed(this.rs.updatePrecision); // round necessary for not get
                                               // 0.6e-7 for example that
                                               // it's not able for the html
                                               // css width
  ...
  videojs.SelectionBar.prototype._processPlay = function() {
    ...
    if (this.player_.currentTime() >= this.timeStart &&
  ...
  videojs.SelectionBar.prototype.process_loop = function() {
    ...
    if (player && this.looping) {
      ...
      if (current_time < this.timeStart ||
   ...
   * @constructor
   */
  videojs.SelectionBarLeft = videojs.extend(videojs.getComponent('Component'), {
    /** @constructor */
    constructor : function(player, options) {
      Component.call(this, player, options);
      this.on('mousedown', this.onMouseDown);
  ...
  videojs.SelectionBarRight =
      videojs.extend(videojs.getComponent('Component'), {
        /** @constructor */
        constructor : function(player, options) {
          Component.call(this, player, options);
          this.on('mousedown', this.onMouseDown);
   ...
   * @constructor
   */
  videojs.TimePanel = videojs.extend(videojs.getComponent('Component'), {
    /** @constructor */
    constructor : function(player, options) { Component.call(this, player, options); }
  });
   ...
   * @constructor
   */
  videojs.TimePanelLeft = videojs.extend(videojs.getComponent('Component'), {
    /** @constructor */
    constructor : function(player, options) { Component.call(this, player, options); }
  });
   ...
   * @constructor
   */
  videojs.TimePanelRight = videojs.extend(videojs.getComponent('Component'), {
    /** @constructor */
    constructor : function(player, options) { Component.call(this, player, options); }
  });
   ...
   * @constructor
   */
  videojs.ControlTimePanel = videojs.extend(videojs.getComponent('Component'), {
    /** @constructor */
    constructor : function(player, options) { Component.call(this, player, options); }
  });
